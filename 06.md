# 06
This document describes the script used to generate Fig. 3A–F and Supp Fig. 3.
...
#Download the official Dorado binary.
#dorado -v
#0.7.1
...
# Create a BED file of promoters (2 kb upstream of the 5′ end on both strands)
...
# Create a BED file of intergenic regions (excluding exons, introns, promoters, and genes)
...
# Note: Annotation names for each transposable element#
>>> df["name"].unique()
...
# Note: TE methylation levels by category
...
#methylation level of exons and introns
...
#Note: methylation level of exons and introns
...
#methylation level of promoter, gene and intergenic regions.
...
#deleate "file_x_file_y_"
expr["gene_id"] = expr["gene_id"].str.replace(r"^file_\d+_file_\d+_", "", regex=True)

# Load the saved gene list
# all_common_og_lists.txt: a file containing JNIG IDs of orthogroups conserved across 33 species, derived from OrthoFinder output
...
# Note: Is the methylation level significantly different between conserved and other genes?
...
# Note: Is gene expression significantly different between conserved and other genes?
...
# Note: Calculate coefficients of variation (CVs) for four groups: (conserved/non-conserved)× (hypermethylated/hypomethylated)

df["conserved_gene"] = df["name"].isin(common_og_lists).map({True: "conserved", False: "nonconserved"})
# Define two methylation groups based on percent_m (threshold: 60%)
df["group_pm"] = np.where(df["percent_m"] >= 60, "hyper", "hypo")
# Create four groups by combining conservation status and methylation level
